...
public class NSVAnimatedTabController: UIViewController {
    ...
    public func animateTabbarFoldStatus(isFolded: Bool) {
        ...
        if let centerItemMove = mainOptions.animationOptions.tabMovePercentage {
            if !isOpen {
                centerItemBottomConstraint?.constant = -mainOptions.tabInsets.bottom
            } else {
                centerItemBottomConstraint?.constant = view.bottomPadding + (animatedTab.frame.height*centerItemMove)
            }
        }
        if isFolded {
